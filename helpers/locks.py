...
async def _can_manage_locks(update: Update) -> bool:
    """تغيير الأقفال للمميز/الادمن؟ نرفع الصلاحية إلى ادمن فما فوق."""
    ...
    try:
        ...
        if str(member.status) in ("administrator", "owner", "creator"):
            return True
...
async def _lock_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    m = re.fullmatch(r"(قفل|فتح)\s+(.+?)", text)
    if not m:
        return
